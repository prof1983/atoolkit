...
uses
  ...
  Calendar;
  {$ELSE}
  Classes, Controls, Forms, Graphics, Grids, Messages, StdCtrls, SysUtils, Windows;
  {$ENDIF}

{$IFDEF FPC}
type
  TCalendar = Calendar.TCalendar;
{$ELSE}
type
  TDayOfWeek = 0..6;
...
begin
  if ARow = 0 then  { day names at tops of columns }
    Result := {$IFDEF DELPHI_XE_UP}FormatSettings.{$ENDIF}ShortDayNames[(StartOfWeek + ACol) mod 7 + 1]
  else
  begin
